#!/usr/bin/env python3
"""
Fast Patient Readmission Risk Prediction Model
...
class FastReadmissionModel:
    ...
    def create_evaluation_plots(self, y_test, save_path='models/'):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}readmission_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}readmission_roc_curve.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if self.feature_importance is not None:
            ...
            plt.tight_layout()
            plt.savefig(f'{save_path}readmission_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
...
def main():
    ...
    logger.info("🚀 Starting Fast Readmission Risk Prediction Model Training")
    logger.info("="*80)
    
    # Initialize the model trainer
    trainer = FastReadmissionModel()
    ...
    comparison_df.to_csv('models/readmission_model_comparison_results.csv', index=False)
    logger.info("📄 Model comparison results saved to models/readmission_model_comparison_results.csv")
